...
from consultant_core.models import AuthConsultantDocument, IdentityDocument, ConsultantCredential, ConsultantDocument

try:
    from consultant_assessment.models import FaceVerification, UserSession, VideoResponse, Violation, ProctoringSnapshot
except ImportError:
...
def consultant_list(request):
    ...
    for u in users:
        ...
        latest_face = FaceVerification.objects.filter(user=u).order_by('-verified_at').first()
        if latest_face:
            face_status = 'Matched' if latest_face.is_match else 'No Match'
        else:
            face_status = 'Not Done'

        # Document verification status (IdentityDocument + ConsultantDocument)
        identity_docs = IdentityDocument.objects.filter(user=u)
        consultant_docs_qs = ConsultantDocument.objects.filter(user=u)
...
from consultant_core.utils import generate_and_send_credentials
